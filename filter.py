...
path = 'D://filter_test/'
# path = 'C://Users/li/Desktop/FCN-tensorflow-hzp-master/logs/'
...
def GetResult(index):  # 将原图与预测标签图对应相乘，得分割图
    ...
    for i in range(224):
        for j in range(224):
            ...
            elif pred_np[i][j] == 0:  # 分类为新城区，红色
                result_pred[i][j][0] = int(alpha * ini[i][j][0])
                result_pred[i][j][1] = int(alpha * ini[i][j][1])
                result_pred[i][j][2] = int(alpha * ini[i][j][2])
                # result_pred[i][j] = [0, 0, 0]
            else:  # 分类为老城区，蓝色
                result_pred[i][j][0] = int(beta * ini[i][j][0])
                result_pred[i][j][1] = int(beta * ini[i][j][1])
                result_pred[i][j][2] = int(beta * ini[i][j][2])
