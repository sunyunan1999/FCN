...
def get_variable(weights, name):
    init = tf.constant_initializer(weights, dtype=tf.float32)  #可能有问题
...
def conv2d_basic(x, W, bias):
    conv = tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding="SAME")  # W是卷积核,[height高度,width宽度,in_channel卷积核个数，out_channel核个数]
    return tf.nn.bias_add(conv, bias)  # 加b
