...
def vgg_net(weights, image):
    ...
    for i, name in enumerate(layers):
        ...
        if kind == 'conv':
            ...
            kernels = utils.get_variable(np.transpose(kernels, (1, 0, 2, 3)), name=name+"_w")  # k值转置  #可能有问题
...
def inference(image, keep_prob):  # 输入图像和dropout值
    ...
    with tf.variable_scope("inference"):
        ...
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3], NUM_OF_CLASSESS], name="W_t1")# 扩大两倍  所以stride = 2  kernel_size = 4
        ...
        W_t2 = utils.weight_variable([4, 4, deconv_shape2[3], deconv_shape1[3]], name="W_t2")# 输出通道数为pool3通道数，输入通道数为pool4通道数
        ...
        W_t3 = utils.weight_variable([16, 16, NUM_OF_CLASSESS, deconv_shape2[3]], name="W_t3")
    ...
    if mode == "train":
        ...
        for sx in range(1,201):
          ...
          #  test_image = misc.imread('D://FMCW_Interference/test/inp_'+str(sx)+'.jpg')
            test_image = misc.imread('D:/Data/deeplearning/FCN/Data_zoo/MIT_SceneParsing/ADEChallengeData2016/images/FMCW_Interference/test/inp_'+str(sx)+'.jpg')
            ...
            utils.save_image(pred[0].astype(np.uint8),'D:/Data/deeplearning/FCN/Data_zoo/MIT_SceneParsing/ADEChallengeData2016/images/FMCW_Interference/test/', name="pred_" + str(sx))
